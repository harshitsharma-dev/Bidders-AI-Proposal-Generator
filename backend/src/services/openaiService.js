...
class OpenAIService {
        ...
        try {
            ...
            const completion = await this.openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: "system",
                        content: "You are a professional business proposal writer with expertise in creating compelling, detailed proposals for government and corporate tenders. Generate well-structured, professional proposals that highlight company strengths and address specific tender requirements."
                    },
                    {
                        role: "user",
                        content: prompt
            ...
            if (error.status === 429) {
                console.log('Rate limit hit, waiting and retrying with shorter prompt...');
                ...
                try {
                    const completion = await this.openai.chat.completions.create({
                        ...
                        messages: [
                            {
                                role: "user",
                                content: `Write a professional business proposal for tender: "${tender.title}". Company: ${companyProfile.companyName} with ${companyProfile.experience} years experience. Budget: $${tender.budget}. Requirements: ${tender.requirements ? tender.requirements.join(', ') : 'Various'}. Include executive summary, technical approach, qualifications, and timeline.`
                            }
                        ],
                        max_tokens: 1000,
                        temperature: 0.5,
    ...
    createProposalPrompt(tender, profile) {
        return `
            ...
            Capabilities: ${profile.capabilities ? profile.capabilities.join(', ') : 'Not specified'}
            Experience: ${profile.experience} years
            Completed Projects: ${profile.completedProjects}
            Success Rate: ${profile.successRate}%
            Certifications: ${profile.certifications ? profile.certifications.join(', ') : 'None specified'}
