...
class TenderAggregatorService {
  ...
  async fetchAllTenders(countries = ['usa', 'uk', 'canada', 'australia']) {
    ...
    console.log(`✅ Fetched ${allTenders.length} tenders from ${countries.length} countries`);
    return allTenders;
  }

  async searchTenders(query, countries = ['usa', 'uk', 'canada', 'australia'], filters = {}) {
    ...
    if (query && query.trim()) {
      ...
      filteredTenders = filteredTenders.filter(tender => 
        ...
        tender.requirements.some(req => req.toLowerCase().includes(queryLower)) ||
    ...
    if (filters.region) {
      filteredTenders = filteredTenders.filter(tender => 
        tender.region && tender.region.toLowerCase().includes(filters.region.toLowerCase())
      );
    }
  ...
  async getTendersByCountry(country) {
    ...
    if (!this.apis[countryLower]) {
      throw new Error(`Unsupported country: ${country}. Supported countries: ${Object.keys(this.apis).join(', ')}`);
    }
  ...
  async getTendersByLocation(location) {
    ...
    return allTenders.filter(tender => {
      ...
      return (
        (tender.location.city && tender.location.city.toLowerCase().includes(locationLower)) ||
        (tender.location.state && tender.location.state.toLowerCase().includes(locationLower)) ||
        (tender.location.province && tender.location.province.toLowerCase().includes(locationLower)) ||
        (tender.location.region && tender.location.region.toLowerCase().includes(locationLower)) ||
  ...
  async getRecommendations(companyProfile) {
    ...
    const recommendedTenders = allTenders.map(tender => {
      const matchingRequirements = tender.requirements.filter(req =>
        companyProfile.capabilities.some(cap =>
          cap.toLowerCase().includes(req.toLowerCase()) ||
          req.toLowerCase().includes(cap.toLowerCase())
        )
      );

      const capabilityMatch = matchingRequirements.length / Math.max(tender.requirements.length, 1);
      const budgetMatch = this.calculateBudgetMatch(tender.budget, companyProfile);
      const locationMatch = this.calculateLocationMatch(tender, companyProfile);
      
      const overallSimilarity = (capabilityMatch * 0.5) + (budgetMatch * 0.3) + (locationMatch * 0.2);
  ...
  calculateBaseSimilarity(tender) {
    ...
    const techKeywords = ['AI', 'Technology', 'Software', 'Digital', 'Cloud', 'Data'];
    const hasTechKeywords = techKeywords.some(keyword =>
      ...
      tender.requirements.some(req => req.toLowerCase().includes(keyword.toLowerCase()))
    );
  ...
  generateMatchReasons(tender, profile, matches) {
    ...
    if (matches.capabilityMatch > 0.7) {
      reasons.push(`Strong capability match: Your expertise in ${profile.capabilities.slice(0, 3).join(', ')} aligns well`);
    }
  ...
  async getTenderStats() {
    ...
    allTenders.forEach(tender => {
      // By country
      stats.byCountry[tender.country] = (stats.byCountry[tender.country] || 0) + 1;
      ...
      // By region
      if (tender.region) {
        stats.byRegion[tender.region] = (stats.byRegion[tender.region] || 0) + 1;
      }
