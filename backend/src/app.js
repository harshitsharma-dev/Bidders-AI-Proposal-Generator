...
app.get('/api/models', async (req, res) => {
  try {
    ...
    const models = [
      ...
      { name: 'gpt-4-turbo', description: 'Latest GPT-4 with better performance' }
    ];
...
app.post('/api/proposals/generate', async (req, res) => {
  try {
    ...
    const proposal = await openaiServiceInstance.generateProposal(tender, company);
...
app.get('/api/tenders/search', auth, (req, res) => TenderController.searchTenders(req, res));
app.get('/api/tenders/recommendations', auth, (req, res) => TenderController.getRecommendations(req, res));
app.get('/api/tenders/stats', auth, (req, res) => TenderController.getTenderStats(req, res));
app.get('/api/tenders/country/:country', auth, (req, res) => TenderController.getTendersByCountry(req, res));
app.get('/api/tenders/location/:location', auth, (req, res) => TenderController.getTendersByLocation(req, res));
app.get('/api/tenders/api/countries', auth, (req, res) => TenderController.getCountries(req, res));
...
app.get('/api/tenders/:id', auth, (req, res) => TenderController.getTenderById(req, res));
...
app.post('/api/payments/create-customer', async (req, res) => {
  try {
    ...
    const customer = await paymentService.createCustomer(email, name, companyName);
...
app.post('/api/payments/create-subscription', async (req, res) => {
  try {
    ...
    const subscription = await paymentService.createSubscription(customerId, priceId, metadata);
...
app.listen(PORT, () => {
  ...
  console.log(`🔥 Firebase: ${process.env.FIREBASE_PROJECT_ID ? 'Configured' : 'Missing Config'}`);
});
